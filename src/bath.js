...
function renderBath() {
  const bathGrid =
    document.getElementById("bath-grid");
  ...
  allBath.forEach((product) => {
    const price =
      Math.round(product.price * 15000);
    ...
    bathGrid.innerHTML += `
      <div data-product-id="${product.id}" class="flex flex-col h-full p-3 hover:shadow-[0px_0px_5px_black] rounded-xl cursor-pointer">
        ...
        <p class="text-[#668c4a] font-semibold">Rp${price.toLocaleString("id-ID")}</p>
      ...
      const bathProducts =
        data.products.filter(
          (product) => {
            ...
            return (
              searchText.includes("bath") ||
              searchText.includes("towel") ||
              searchText.includes("soap") ||
              searchText.includes("shower") ||
              searchText.includes("body") ||
              searchText.includes("skin") ||
